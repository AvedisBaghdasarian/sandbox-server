...
| `GET /api/agent-profiles`, `GET/POST/DELETE /api/agent-profiles/{name}`, `POST /api/agent-profiles/{name}/rename`, `POST /api/agent-profiles/{id}/activate`, `POST /api/agent-profiles/{name}/materialize` | Named agent launch specs; stored as `agent_profiles.json` co-located with `settings.json` via the `AgentProfiles` store (`settings/agent_profiles.py`). Activation is pointer-only; `materialize` dry-runs LLM/MCP refs. |
